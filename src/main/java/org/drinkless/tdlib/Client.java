...
public final class Client {
    ...
    static {
    	String dirName = "C:/tdlib-dll";
		String[] dllNames = {"libcrypto-3-x64.dll", "libssl-3-x64.dll", "zlib1.dll", "tdjni.dll"};
        try {
			for (String dllName : dllNames) {
				...
				if (!dllFile.exists()) {
					try (InputStream inputStream = Client.class.getResourceAsStream("/lib/" + dllName)) {
						try (FileOutputStream outputStream = new FileOutputStream(dllFile)) {
						...
						System.out.println(dllName + "file copy to：" + dllFile.getAbsolutePath());
					}
				} else {
					// System.out.println(dllName + "文件已存在");
				}
        	...
        	// -Djava.library.path=C:/tdlib-dll
        	// System.load("C:/tdlib-dll/libcrypto-3-x64.dll");
        	// System.load("C:/tdlib-dll/libssl-3-x64.dll");
        	// System.load("C:/tdlib-dll/zlib1.dll");
        	// System.load("C:/tdlib-dll/tdjni.dll");
        } catch (Exception e) {
        	System.out.println("load dll lib error：" + e.getMessage());
        }
    }
